...
const AutoComplete = ({ allPokemons, setDisplayedPokemons }) => {
  ...
  const filterNames = (value) => {
    ...
    return name ? allPokemons.filter((e) => e.name.includes(name)) : []
  }

  const checkEqualName = (value) => {
    const filteredArray = filterNames(value)

    return filteredArray[0]?.name === value ? [] : filteredArray
  }
